...
Popup.prototype = {
	...
	iniBuild : function() {
		G('dialogCase') ? G('dialogCase').parentNode
				.removeChild(G('dialogCase')) : function() {
		};
	...
	build : function() {
		...
		var path = "dialogclose.gif";
		var close = '<a id="dialogBoxClose" href="#" border="0" width="16" height="16" align="absmiddle" alt="关闭">'
				+ (this.config.showCloseButton ? 'Ｘ' : '') + '</a>';
		var cB = 'filter: alpha(opacity=' + this.info.coverOpacity
		...
		var cover = '<div id="dialogBoxBG" style="position:absolute;top:0px;left:0px;width:100%;height:100%;z-index:'
				...
				+ cB
				+ 'background-color:'
				+ this.color.cColor + ';display:none;"></div>';
		var mainBox = '<div id="dialogBox" style="border:1px solid '
				+ this.color.tColor
				+ ';display:none;z-index:'
				+ showZIndex
				+ ';position:relative;width:'
				+ this.config.width
				+ 'px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="'
				+ this.color.bColor + '">';
		if (this.config.isHaveTitle) {
			mainBox += '<tr height="24" bgcolor="'
					+ this.color.tColor
					+ '"><td><table style="-moz-user-select:none;height:24px;" width="100%" border="0" cellpadding="0" cellspacing="0" ><tr>'
					+ '<td width="6" height="24"></td><td id="dialogBoxTitle" style="color:'
					+ this.color.wColor
					+ ';font-size:14px;font-weight:bold;">'
					+ this.info.title
					+ '&nbsp;</td>'
					+ '<td id="dialogClose" width="20" align="right" valign="middle">'
					+ close + '</td><td width="6"></td></tr></table></td></tr>'
		} else {
			mainBox += '<tr height="10"><td align="right">' + close
					+ '</td></tr>'
		}
		;
		mainBox += '<tr style="height:'
				+ this.config.height
				+ 'px" valign="top"><td id="dialogBody" style="position:relative;"></td></tr></table></div>'
				+ '<div id="dialogBoxShadow" style="display:none;z-index:'
				+ baseZIndex + ';"></div>';
		...
		Event.observe(G('dialogBoxClose'), "click", this.reset
	...
	setBackgroundSize : function() {
		var getValueWidth;
		var getMaxValueWidth = [ getValue("clientWidth"),
				getValue("scrollWidth") ];
		getValueWidth = eval("Math.max(" + getMaxValueWidth.toString() + ")");
		G('dialogBoxBG').style.width = getValueWidth;
		var getValueHeight;
		var getMaxValueHeight = [ getValue("clientHeight"),
				getValue("scrollHeight") ];
	...
	lastBuild : function() {
		var confirm = '<div style="width:100%;height:100%;text-align:center;"><div style="margin:20px 20px 0 20px;font-size:14px;line-height:16px;color:#000000;">'
				...
				+ '</div><div style="margin:20px;"><input id="dialogOk" type="button" value=" 确定 "/>&nbsp;<input id="dialogCancel" type="button" value=" 取消 "/></div></div>';
		var alert = '<div style="width:100%;height:100%;text-align:center;"><div style="margin:20px 20px 0 20px;font-size:14px;line-height:16px;color:#000000;">'
				+ this.info.alertCon
		...
		if (this.config.contentType == 1) {
			var openIframe = "<iframe width='100%' style='height:"
					...
					+ "' id='" + this.iframeIdName + "' src='"
					+ this.info.contentUrl + "' frameborder='0' scrolling='"
					+ this.config.scrollType + "'></iframe>";
			...
			Event.observe(G('dialogOk'), "click", this.forCallback
					.bindAsEventListener(this), false);
			Event.observe(G('dialogCancel'), "click", this.close
					.bindAsEventListener(this), false);
		} else if (this.config.contentType == 4) {
			G("dialogBody").innerHTML = alert;
			Event.observe(G('dialogYES'), "click", this.close
					.bindAsEventListener(this), false);
		}
		;
	},
	...
	middle : function() {
		...
		var iTop = (sClientHeight / 2 + sScrollTop)
	...
	hiddenTag : function(tagName) {
		...
		if (ele != null) {
			for (var j = 0; j < ele.length; j++) {
				if (ele[j].name != "tbSelYear" && ele[j].name != 'tbSelMonth')
	...
	showSome : function() {
		for (var j = 0; j < this.someToHidden.length; j++) {
			if (this.someToHidden[j].name != 'tbSelMonth'
					&& this.someToHidden[j].name != 'tbSelYear')
...
Dragdrop.prototype = {
	initialize : function(width, height, shadowWidth, showShadow, contentType) {
		...
		Event.observe(G('dialogBoxTitle'), "mousedown", this.moveStart
				.bindAsEventListener(this), false);
	},
	...
	mousemove : function(event) {
		if (!this.IsDraging)
			return;
		var iLeft = Event.pointerX(event) - this.dragData["x"]
				+ parseInt(this.oObj.style.left);
		var iTop = Event.pointerY(event) - this.dragData["y"]
				+ parseInt(this.oObj.style.top);
	...
	mouseup : function(event) {
		...
		var mousY = Event.pointerY(event)
		...
		if (mousX < 1 || mousY < 1 || mousX > document.body.clientWidth
				...
				|| parseInt(this.oObj.style.left) < 0
				|| parseInt(this.oObj.style.top) < 0
				|| oObjRight > document.body.clientWidth
				|| oObjBottom > document.body.clientHeight) {
...
function showMessageBox(title, mess) {
	var strHtml = "<table width='100%' height='80' border='0' cellpadding='0' cellspacing='0' bgcolor='#FFFFFF'><tr><td><div id='show' style=' margin:auto; text-align:center; font-size: 14px'>"
			+ mess + "</div></td></tr></table>";
	g_pop = new Popup({
		contentType : 2,
 ...
 * @param mess
 */
function showReportPrintBox(url, reloadOnClose) {
	g_pop = new Popup({
		contentType : 1,
		showCloseButton : true,
		isReloadOnClose : false,
